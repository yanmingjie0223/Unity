...
public class CVSConfigLoader
{
    public static Dictionary<string, byte[]> ConfigSource = new();

    public static IEnumerator Preload(Action<string> start, Action<float> progress, Action<bool> end)
    {
        AsyncOperationHandle<IList<IResourceLocation>> handle = Addressables.LoadResourceLocationsAsync(BundleTypeName.Config);
...
public abstract class CSVConfigTable<U, T> : ICSVConfig where U : IBaseTable, new()
{
    ...
    {
        var t = typeof(U);
        MemoryStream tableStream = new(CVSConfigLoader.ConfigSource[t.Name]);
        using var reader = new StreamReader(tableStream, Encoding.GetEncoding(936));
        ...
        var tempSB = new StringBuilder();
        var fieldNameArray = CVSConfigLoader.ParseLine(fieldNameStr, tempSB);
        ...
        while (lineStr != null)
        {
            var itemStrArray = CVSConfigLoader.ParseLine(lineStr, tempSB);
